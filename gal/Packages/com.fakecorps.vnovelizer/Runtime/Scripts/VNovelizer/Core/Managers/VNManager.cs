...
// 正式播放
    ...
    /// <summary>
    /// 清除跨场景的待处理状态（防止场景重载时意外触发新游戏/读档）
    /// </summary>
    public void ClearPendingSceneState()
    {
        pendingScriptName = null;
        pendingLineID = null;
        pendingSaveData = null;
    ...
    /// <summary>
    /// 【Jump2Pac修复】重置 VNManager 到主菜单初始状态
    /// 停止所有协程，清空剧本/角色/BGM/特效/跨场景数据
    /// </summary>
    public void ResetForMainMenu()
    {
        // 1. 安全停止剧情推进协程（宿主可能已随场景销毁）
        if (_flowCoroutine != null)
        {
            try { MonoManager.GetInstance().StopCoroutine(_flowCoroutine); }
            catch (System.Exception) { /* 宿主已销毁，忽略 */ }
            _flowCoroutine = null;
        }
        // 2. 安全停止自动播放协程
        if (_autoPlayCoroutine != null)
        {
            try { MonoManager.GetInstance().StopCoroutine(_autoPlayCoroutine); }
            catch (System.Exception) { /* 宿主已销毁，忽略 */ }
            _autoPlayCoroutine = null;
        }
        // 3. 重置自动播放/跳过/文本显示状态
        ...
        isReplayMode = false;
        replayEndLineID = "";
        wasMainMenuVisibleBeforeReplay = false;
        // 5. 清空跨场景待处理数据
        ClearPendingSceneState();
        // 6. 清空当前剧本数据
        currentScriptName = null;
        StoryLines.Clear();
        LineIDIndexMap.Clear();
        CurrentLineIndex = -1;
        lastLine = null;
        // 7. 清空运行时状态
        currentBG = null;
        ...
        // 8. 清理回调
        onGameStartedCallback = null;
        // 注意：不重置 isListeningSceneLoad，因为构造函数只执行一次
        // sceneLoaded 监听需要持续有效以响应后续的 VNGamePlay 场景加载
        Debug.Log("[VNManager] 已重置为 MainMenu 状态");
    }
