...
public class GameStateManager : BaseManager<GameStateManager>
{
    ...
    public bool CanOpenPanel(GameState targetState)
    {
        ...
        // 【新增】如果当前状态是SaveLoad，允许打开Settings面板（主菜单跨面板切换）
        if (currentState == GameState.SaveLoad)
        {
            if (targetState == GameState.Settings)
                return true;
        }
        
        // 【新增】如果当前状态是Settings，允许打开SaveLoad面板（主菜单跨面板切换）
        if (currentState == GameState.Settings)
        {
            if (targetState == GameState.SaveLoad)
                return true;
        }
        
        // 【新增】如果当前状态是History，允许打开SaveLoad和Settings面板（主菜单跨面板切换）
        if (currentState == GameState.History)
        {
            if (targetState == GameState.SaveLoad || targetState == GameState.Settings)
                return true;
        }
        
        // 如果当前状态是其他面板状态，不允许打开新面板
        return false;
    }
    ...
    public void ResetToMainMenu()
    {
        ...
        Debug.Log("[GameState] 已重置为 MainMenu 状态 (Gameplay)，状态栈已清空");
        EventCenter.GetInstance().EventTrigger("GameStateChanged", currentState);
    }
}